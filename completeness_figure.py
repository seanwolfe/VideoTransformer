...
if 'classifier' in which:
    ...
    # Load the CSV file
    file_path = 'inference_results_reg.csv'  # Replace with your file path if different
    data = pd.read_csv(file_path)
    ...
    # Group by the bins and calculate accuracy for each bin
    binned_data = data.groupby(['v_bin', 'omega_bin']).agg(
        pos_mae=('pos_mae', 'mean')
    ).reset_index()
    binned_data['pos_mae'] = int(binned_data['pos_mae'] * 224 * 0.66 * 1000)
    ...
    # Create the heatmap
    plt.figure(figsize=(12, 8))
    ax = sns.heatmap(heatmap_matrix, annot=True, cmap='Purples', cbar_kws={'label': 'Position MAE [mas]'},
                     linewidths=0.5, fmt='.1f', linecolor='white')
